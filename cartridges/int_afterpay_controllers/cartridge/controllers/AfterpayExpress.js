...
function CreateToken(){
    if (! afterpayEnabled || !expressCheckoutEnabled) {
        responseUtils.renderJSON({ status: 'FAILURE', error: Resource.msg('expresscheckout.error.disabled', 'afterpay', null) });
        return;
    }
    ...
    if (!cart) {
        AfterpaySession.clearSession();
        responseUtils.renderJSON({ status: 'FAILURE', error: Resource.msg('expresscheckout.error.emptycart', 'afterpay', null)});
        return;
    }
    ...
    if ((numHomeDeliveries == 0) && (Object.keys(storeMap).length == 1)) {
        ...
        responseUtils.renderJSON({ status: 'FAILURE', error: Resource.msg('expresscheckout.error.multidestination', 'afterpay', null)});
        return;
    }
    ...
    if (! COHelpers.isPriceWithinThreshold(createCheckoutPrice)) {
        responseUtils.renderJSON({ status: 'FAILURE'
            ,error: Resource.msgf('minimum.threshold.message', 'afterpay', null, new dw.value.Money(sitePreferences.getMinThresholdAmount(), cart.object.currencyCode)
             , new dw.value.Money(sitePreferences.getMaxThresholdAmount(), cart.object.currencyCode))});
        return;
    }
    
    // Need to pass the amount in as well, not just compute it from the cart.
    var afterPayTokenResponse = require('*/cartridge/scripts/checkout/AfterpayExpressGetToken').GetToken(cart, createCheckoutPrice, sourceUrl, merchantOrderNum, store);
    if (afterPayTokenResponse.error) {
        responseUtils.renderJSON({ status: 'FAILURE', error: Resource.msg('expresscheckout.error.gettoken', 'afterpay', null) });
        return;
    }
...
function GetShippingMethods() {
    ...
    var i, address, applicableShippingMethods, currentShippingMethod, method;
    var cart = app.getModel('Cart').get();
    var TransientAddress = app.getModel('TransientAddress');

    if (!cart) {
        Logger.error(Resource.msg('expresscheckout.error.emptycart', 'afterpay', null));
        AfterpaySession.clearSession();
        responseUtils.renderJSON([]);
        return;
    }
...
function PostAfterpayCheckoutFlow() {
    ...
    if (!apOrderToken) {
        ...
        redirectToErrorDisplay(Resource.msg('expresscheckout.error.invalidsession', 'afterpay', null));
        return;
    }
    
    // retrieve the order from Afterpay using api
    var afterPayOrderResponse = require('*/cartridge/scripts/checkout/AfterpayGetOrder').GetOrder(apOrderToken);
    ...
    //    redirectToErrorDisplay(Resource.msg('expresscheckout.error.checkout', 'afterpay', null));
    //    return;
    //}
    ...
    if (expressCheckoutShippingStrategy == "deferred") {
        return DeferredShippingFlow(afterPayOrderResponse);
    }
    else {
        return IntegratedShippingFlow(afterPayOrderResponse);
    }
}
...
function DeferredShippingFlow(afterPayOrderResponse) {
    ...
    if (!cart) {
        AfterpaySession.clearSession();
        responseUtils.renderJSON({ type: 'DeferredShippingFlow', error: Resource.msg('expresscheckout.error.emptycart', 'afterpay', null) });
        return;
    }
    ...
    Transaction.wrap(function () {
        ...
        let paymentInstrument = cart.object.createPaymentInstrument('AFTERPAY_PBI', new Money(0.0, cart.object.currencyCode));
    ...
    app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.htmlValue = 'AFTERPAY_PBI';
    app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.value = 'AFTERPAY_PBI';
...
function IntegratedShippingFlow(afterPayOrderResponse) {
    ...
    if (!cart) {
        AfterpaySession.clearSession();
        responseUtils.renderJSON({ type: 'IntegratedShippingFlow', error: Resource.msg('expresscheckout.error.emptycart', 'afterpay', null) });
        return;
    }
    ...
    if (buyNow) {
        ...
        require('int_afterpay_core/cartridge/scripts/checkout/AfterpayUpdatePreapprovalStatus').GetPreApprovalResult(cart.object, {status: "SUCCESS", apToken: apOrderToken, apExpressCheckout: true});
        ...
        Transaction.wrap(function () {
            ...
            let paymentInstrument = cart.object.createPaymentInstrument('AFTERPAY_PBI', new Money(amount, currency));
            COHelpers.addShippingAddressToBasket(cart.object, afterPayOrderResponse.shipping);
        });
        // Prepopulate the billing form with Afterpay payment type
        app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.htmlValue = 'AFTERPAY_PBI';
...
function ContinueFinalize() {
    ...
    if (!cart) {
        AfterpaySession.clearSession();
        responseUtils.renderJSON({ type: 'ContinueFinalize', error: Resource.msg('expresscheckout.error.emptycart', 'afterpay', null) });
        return;
    }
...
function FinalizeOrder() {
    ...
    if (!AfterpaySession.isExpressCheckoutFinalizeFlow()) {
        ...
        AfterpaySession.clearSession();
        redirectToErrorDisplay(Resource.msg('expresscheckout.error.notfinalizeflow', 'afterpay', null));
        return;
    }
    ...
    // needed so checkout thinks we're past billing stage
    session.forms.billing.fulfilled.value = true;
    require('int_afterpay_core/cartridge/scripts/checkout/AfterpayUpdatePreapprovalStatus').GetPreApprovalResult(cart.object, {status: "SUCCESS", apToken: apOrderToken, apExpressCheckout: true, apExpressCheckoutChecksum: widgetChecksum});
...
function DebugOrder() {
    ...
    if (action === 'showsession') {
        let sess = AfterpaySession.debugGetSessionAsString();
