...
var createAfterpaySitePreferencesUtilities = Class.extend({
    ...
    getRedirectConfirmUrl: function() {
        return dw.web.URLUtils.https('AfterpayRedirect-HandleResponse').toString();
    },
	...
	isExpressCheckoutEnabled: function() {
		return Site.getCurrent().getCustomPreferenceValue('apEnableExpressCheckout');
	},
	getExpressCheckoutJS: function() {
		return Site.getCurrent().getCustomPreferenceValue('apExpressCheckoutJS');
	},
...
var disableSummaryForAfterpay = function(cart, viewContext) {
	...
	while (iter.hasNext()) {
		...
		if ((expressCheckoutEnable && isExpressCheckout) || afterpayEnable == false || apPaymentInstrument.paymentMethod != 'AFTERPAY_PBI') {
