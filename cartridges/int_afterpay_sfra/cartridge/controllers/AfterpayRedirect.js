 ...
 */
 server.get('PrepareRedirect', server.middleware.https, function (req, res, next) {
     ...
     var countryCodeValue = req.querystring.countryCode;
     ...
     if ((afterPayToken !== 'undefined') && countryCodeValue) {
         res.render('checkout/afterpayRedirect', { apToken: afterPayToken, countryCode: countryCodeValue });
     } else {
...
/** saves afterpay payment method in payment instrument */
 server.post('IsAfterpay',
     ...
     function (req, res, next) {
         ...
         var afterpayInstruments = currentBasket.getPaymentInstruments('AFTERPAY_PBI');
         ...
         res.json({
             ...
             resource: {
                 pleaseWait: Resource.msg('redirect.message', 'afterpay', null),
                 redirectMessage: Resource.msg('redirect.notification', 'afterpay', null)
             }
         });
         return next();
     });
